...
program
    ...
    const generator = new Generator(path.resolve(process.cwd(), options.specDir), path.resolve(process.cwd(), options.outDir));
...
program
    ...
    .option('-o, --out-dir <dir>', 'Output directory for generated files', 'generated')
    .action(async (options) => {
    const mockGenerator = new MockGenerator(path.resolve(process.cwd(), options.specDir), path.resolve(process.cwd(), options.outDir));
...
program
    ...
    .action(async (options) => {
    const server = new Server(path.resolve(process.cwd(), options.specDir), path.resolve(process.cwd(), options.outDir), parseInt(options.port, 10));
    await server.start();
});
program.parse();
