...
describe('Generator', () => {
  ...
  const mockApiSpec: Partial<OpenAPIV3.Document> = {
    paths: {
      '/users': {
        get: {
  ...
  describe('generate', () => {
    it('should generate API files for each spec', async () => {
      ...
      expect(fs.mkdirSync).toHaveBeenCalledWith(
        expect.stringContaining(path.join(mockOutDir, 'api', 'routes')),
        { recursive: true }
      );
      
      expect(fs.mkdirSync).toHaveBeenCalledWith(
        expect.stringContaining(path.join(mockOutDir, 'api', 'controllers')),
        { recursive: true }
      );
      
      // Check if files are written
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        expect.stringContaining(path.join(mockOutDir, 'api', 'routes', 'index.js')),
        expect.stringContaining('router.get')
      );
      
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        expect.stringContaining(path.join(mockOutDir, 'api', 'controllers', 'index.js')),
        expect.stringContaining('export const getUsers')
      );
    });

    it('should skip when no paths are defined', async () => {
      (SwaggerParser.parse as any).mockResolvedValue({});
  ...
  describe('generateOperationId', () => {
    it('should generate proper operation IDs for simple paths', () => {
      ...
      const operationId = (generator as any).generateOperationId('get', '/users');
      expect(operationId).toBe('getusers');
    });

    it('should generate proper operation IDs for paths with ID parameters', () => {
      // Use any to access private method
      const operationId = (generator as any).generateOperationId('get', '/users/{userId}');
  ...
  describe('generateControllerFunction', () => {
    it('should generate GET controller for collection', () => {
      // Use any to access private method
      const controller = (generator as any).generateControllerFunction({
        operationId: 'getUsers',
        method: 'get',
        parameters: [],
