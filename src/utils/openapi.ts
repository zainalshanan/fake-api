 ...
 */
export function extractSchemaRef(responses: OpenAPIV3.ResponsesObject): string | null {
  ...
  if (resp && (resp as any).content && (resp as any).content['application/json']) {
    ...
    if (schema) {
      ...
      } else if ((schema as any).type === 'array' && (schema as any).items && (schema as any).items.$ref) {
 ...
 */
export function findOpenApiPath(requestPath: string, paths: Record<string, any>): string {
  const cleanPath = requestPath.replace(/\/$/, '');
  for (const openapiPath of Object.keys(paths)) {
    const regex = new RegExp('^' + openapiPath.replace(/\{[^}]+\}/g, '[^/]+') + '$');
 ...
 */
export function castQueryToString(query: Record<string, any>): Record<string, string> {
 ...
 * @returns Record of string values
 */
export function castHeadersToString(headers: Record<string, any>): Record<string, string> {
